...
2. Add your tracker into normal code handling:
   ...
   2. Adjust `GetAllTrackers()` in `/internal/repository/trackers.go` to also be able to handle your tracker technology (add left join, select statement and case for tracker_type in SQL statement + Scan statement + switch case for your tracker type)
...
You can always look at Chirpstack / (Tetra) / Traccar Implementations for reference.